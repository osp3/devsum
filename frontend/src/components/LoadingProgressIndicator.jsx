...
const LoadingProgressIndicator = ({ 
  ...
  const validProgress = progress !== null ? Math.max(0, Math.min(100, progress)) : null;

  return (
    <div className="flex flex-col items-center justify-center p-6 space-y-4">
      {/* Spinner */}
      {showSpinner && (
        <div className={`animate-spin rounded-full border-2 border-gray-600 border-t-[#5b56dd] ${sizeClasses[size]}`}>
        </div>
      )}
      ...
      {showProgressBar && (
        <div className="w-full max-w-xs bg-gray-700 rounded-full h-2 overflow-hidden">
          {validProgress !== null ? (
            ...
            <div className="h-full bg-gradient-to-r from-[#5b56dd] to-[#7c71f5] rounded-full animate-pulse w-3/4">
            </div>
          )}
        </div>
      )}
